...
var logger = log4js.getLogger('icetunnel');
...
wst_server_reverse = function(options) {
  ...
  if(options != undefined) {
    ...
    //logger.info("allow_list_file: " + allow_list_file);
  ...
  if(https_flag == "true"){
    ...
    this.httpServer = https.createServer(credentials, function(request, response) {
      ...
      response.write("<html>");
      response.write("<head>");
      response.write("<title>icetunnel</title>");
      response.write("</head>");
      response.write("<body>");
      response.write("icetunnel is running!");
      response.write("</body>");
      response.write("</html>");
      response.end();
    });
    ...
    return function(request){
      ...
      if (uri.query.dst != undefined){
        ...
        if(client_uuid != undefined){
            logger.info("[SYSTEM] WebSocket creation towards " + src_address + " on port " + portTcp + " from client " + client_uuid);
        }
        else{
        ...
        if(allow_list_file != undefined ){
            if(client_uuid != undefined){
                ...
                if (c.length > 0){
                    logger.info("[SYSTEM] --> Client " + client_uuid + " found in the allowlist as " + JSON.stringify(c));
                    var p = c.filter(it => it.port === portTcp);
                }
                else
                    logger.info("[SYSTEM] --> Client " + client_uuid + " not found in the allowlist");
                    
                if(p != undefined && p.length > 0 && p[0].port == portTcp){
                    logger.info("[SYSTEM] --> Port is allowed: " + portTcp);
                    ...
                    logger.info("Port is not allowed: " + portTcp + " so the connection is not authorized");
                    //reject the request
                    request.reject(401, "Request unauthorized!");
                    logger.info("[SYSTEM] --> WS connection closed");                    
                }
            }
            else{
                    logger.info("Client is not specified so the connection is not authorized.");
                    //reject the request
                    request.reject(401, "Request unauthorized!");
                    logger.info("[SYSTEM] --> WS connection closed");                    
